<template>
    <div class="block">
        <el-row>
            <el-col :span="24">
                ...
                <el-date-picker v-model="value1" type="datetime" placeholder="选择时间"> </el-date-picker>              
                 ...
                 <el-button icon="el-icon-menu" style="margin-left: 300px" @click="stopCalc" type="primary">停止计算</el-button>
        ...
        <el-row>
            <el-col :span="12">
                <el-input type="textarea" :rows="10" placeholder="请输入内容" v-model="textarea2"> </el-input>
            </el-col>
            <el-col :span="12">
                <el-input type="textarea" :rows="10" placeholder="请输入内容" v-model="textarea3"> </el-input>
            </el-col>
        </el-row>
    </div>
</template>
...
export default {
    ...
    methods: {
        async calc2() {
            ...
            const res = await fetchApidata('Monitor2/Calc', data, 'get');
            ...
            if (res.isSuccess) {
                ...
                for (var i = 0; i < res.content[2].length; i++) {
                   ...
                   message2 +=
                    'actc'+
                    res.content[0][i].actc +
                        'hkla:' +
                        res.content[1][i].hkla.toFixed(2) +
                        '，' +
                        'torqa:' +
                        res.content[1][i].torqa.toFixed(2) +
                        '，' +
                        'mse:' +
                        res.content[1][i].mse.toFixed(2) +
                        '，' +
                        'sppa:' +
                        res.content[1][i].sppa.toFixed(2) +
                        '，' +
                        'ecd:' +
                    ...
                    message3 +=
                        'hkla:' +
                        res.content[0][i].hkla.toFixed(2) +
                        '，' +
                        'torqa:' +
                        res.content[0][i].torqa.toFixed(2) +
                        '，' +
                        'sppa:' +
                        +res.content[0][i].sppa.toFixed(2) +
                        '，' +
                        'ropa:' +
                        res.content[0][i].ropa.toFixed(2) +
                        '，' +
                        'MDOA:' +
                        res.content[0][i].mdoa.toFixed(2) +
                        '，' +
                        'MDIA:' +
                        res.content[0][i].mdia.toFixed(2) +
                        '，' +
                        'Tvolact:' +
                        res.content[0][i].tvolact.toFixed(2) +
                        '\n';
                }
                for (let i = 0; i < res.content[0].length - 1; i++) {
                    if (res.isSuccess) {
                        setTimeout(() => {
                        let temp = new Map();
                        temp.set('破裂压力(g/cm³)', [res.content[1][i].fracturePressureGrad.toFixed(3),res.content[0][i].deptbitm + '\n' + res.content[1][res.content[1].length - 1].time.split(' ')[1]]); //
                        ...
                        temp.set('漏失压力(g/cm³)', [res.content[1][i].collapsePressureGrad.toFixed(3), res.content[0][i].deptbitm + '\n' + res.content[1][res.content[1].length - 1].time.split(' ')[1]]); //
                        ...
                        temp.set('全烃含量', [res.content[0][i].gasa.toFixed(3), res.content[0][i].deptbitm + '\n' + res.content[1][res.content[1].length - 1].time.split(' ')[1]]);
                        ...
                        temp.set('计算悬重(KN)', [res.content[1][i].hkla.toFixed(3), res.content[0][i].deptbitm + '\n' + res.content[1][res.content[1].length - 1].time.split(' ')[1]]);
                        temp.set('实测悬重(KN)', [res.content[0][i].hkla.toFixed(3), res.content[0][i].deptbitm + '\n' + res.content[1][res.content[1].length - 1].time.split(' ')[1]]);
                        temp.set('计算井底压差', [res.content[1][i].difP3.toFixed(3), res.content[0][i].deptbitm + '\n' + res.content[1][res.content[1].length - 1].time.split(' ')[1]]); //
                        ...
                        temp.set('地层压力预测', [res.content[1][i].pp.toFixed(3), res.content[0][i].deptbitm + '\n' + res.content[1][res.content[1].length - 1].time.split(' ')[1]]); //
                        temp.set('ECD1', [res.content[1][i].ecd.toFixed(2), res.content[0][i].deptbitm + '\n' + res.content[1][res.content[1].length - 1].time.split(' ')[1]]);
                        temp.set('dc指数', [res.content[1][i].dc.toFixed(3), res.content[0][i].deptbitm + '\n' + res.content[1][res.content[1].length - 1].time.split(' ')[1]]); //
